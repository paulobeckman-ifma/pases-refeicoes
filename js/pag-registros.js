...
import { $, $$, esc, ico, modal, aviso, confirmar, pedirTexto, baixarCsv, fmtData, fmtDataHora, normalizar, debounce, uuid, hojeISO, horaDe } from './util.js';
...
export async function render(el, { cabecalho, perfil }) {
  ...
  el.innerHTML = cabecalho('Registros de refeição', 'Cada linha é um almoço registrado. Linhas vermelhas foram registradas sem foto.',
    ...
    <div class="tabela-wrap"><table class="tabela"><thead><tr><th>Data</th><th>Hora</th><th>Aluno</th><th>Curso</th><th>Método</th><th>Foto</th><th>Horário</th><th>Atendente</th></tr></thead>
      <tbody id="r-corpo"><tr><td colspan="8" class="vazio">Carregando…</td></tr></tbody></table></div>
    <div class="paginacao" id="r-pag"></div>`;

  async function carregar() {
    $('#r-corpo').innerHTML = '<tr><td colspan="8" class="vazio">Carregando…</td></tr>';
    lista = await api('refeicoes_listar', { p_ini: periodo.ini, p_fim: periodo.fim, p_incluir_excluidos: $('#r-excl').checked });
    filtrar();
  }
  function filtrar() {
    const q = normalizar($('#r-busca').value.trim()), curso = $('#r-curso').value, met = $('#r-metodo').value;
  ...
  function desenhar() {
    ...
    $('#r-resumo').innerHTML = `<b>${ativos.length}</b> registro(s) · ${new Set(ativos.map((r) => r.a)).size} aluno(s) · ${ativos.filter((r) => r.fs === 'sem_foto').length} sem foto · ${ativos.filter((r) => !r.dh).length} fora do horário · ${esc(periodo.rotulo)}`;
    ...
    $('#r-corpo').innerHTML = pg.map((r) => {
      ...
      return `<tr class="clicavel ${r.ex ? 'excluido' : r.fs === 'sem_foto' ? 'sem-foto' : ''}" data-id="${r.id}">
        ...
        <td class="pequeno">${esc(a.curso || '')}</td><td>${METODO[r.m]}</td>
        ...
        <td>${r.dh ? '<span class="selo verde">dentro</span>' : `<span class="selo ambar">${ico('relogio')} fora</span>`}</td>
        <td class="pequeno">${esc(r.op || '')}${r.off ? '<br><small class="mudo">enviado depois (offline)</small>' : ''}</td></tr>`;
    }).join('') || '<tr><td colspan="8" class="vazio">Nenhum registro encontrado.</td></tr>';
    ...
    const m = modal({
      ...
      corpo: `<div class="fotos-par">
        ...
        <dl class="dl"><dt>Aluno</dt><dd>${esc(a.nome || '')} · ${esc(a.matricula || '')}</dd><dt>Curso</dt><dd>${esc(a.curso || '')}</dd>
          <dt>Data e hora</dt><dd>${fmtData(r.dt)} às ${esc(r.h)} ${r.dh ? '<span class="selo verde">dentro do horário</span>' : '<span class="selo ambar">fora do horário</span>'}</dd>
  ...
  function editar(r) {
    modal({
      titulo: 'Editar registro', corpo: `${seletorAluno(r.a)}
        ...
        <label class="check"><input type="checkbox" data-x ${r.x ? 'checked' : ''}> Refeição extra (segunda no mesmo dia)</label>
      ...
      botoes: [{ texto: 'Cancelar' }, { texto: 'Salvar', classe: 'primario', acao: async (fechar, el) => {
        ...
        const dados = { aluno_id: aid, registrado_em: paraISO($('[data-dh]', el).value), observacao: $('[data-o]', el).value, extra: $('[data-x]', el).checked };
  ...
  function novoManual() {
    ...
    modal({
      titulo: 'Registro manual', corpo: `<div class="caixa aviso-caixa pequeno">Use quando o balcão não pôde ser usado. O registro fica marcado como <b>manual e sem foto</b> e exige justificativa.</div>
        ${seletorAluno()}
        <label class="campo"><span>Data e hora da refeição</span><input type="datetime-local" data-dh value="${agora}"></label>
        ...
        <label class="check"><input type="checkbox" data-x> Refeição extra (o aluno já tem registro nesse dia)</label>`,
      botoes: [{ texto: 'Cancelar' }, { texto: 'Registrar', classe: 'primario', acao: async (fechar, el) => {
        ...
        try {
          const r = await api('registrar_refeicao', { p_id: uuid(), p_aluno_id: aid, p_registrado_em: paraISO($('[data-dh]', el).value),
            p_metodo: 'manual', p_tem_foto: false, p_justificativa: $('[data-j]', el).value, p_extra: $('[data-x]', el).checked });
          if (r.status === 'ok') { aviso(`Registrado: ${r.nome} às ${r.hora}.`, 'ok'); fechar(); carregar(); }
          else if (r.status === 'duplicado') aviso(`Já existe registro nesse dia às ${r.hora}. Marque "refeição extra" se for o caso.`, 'erro');
  ...
  ['#r-curso', '#r-metodo', '#r-semfoto', '#r-fora'].forEach((s) => ($(s).onchange = filtrar));
  ...
  $('#r-csv').onclick = () => baixarCsv(`pases_registros_${periodo.ini}_a_${periodo.fim}`,
    ['data', 'hora', 'aluno', 'matricula', 'curso', 'metodo', 'foto', 'justificativa_sem_foto', 'dentro_do_horario', 'extra', 'atendente', 'offline', 'observacao', 'excluido', 'motivo_exclusao', 'id'],
    filtrada.map((r) => { const a = A.get(r.a) || {}; return [fmtData(r.dt), r.h, a.nome, a.matricula, a.curso, METODO[r.m], r.fs === 'sem_foto' ? 'SEM FOTO' : r.fs, r.j, r.dh ? 'sim' : 'não', r.x ? 'sim' : '', r.op, r.off ? 'sim' : '', r.obs, r.ex ? 'sim' : '', r.exm, r.id]; }));
}
