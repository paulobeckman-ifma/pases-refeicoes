...
import { $, $$, esc, ico, baixarCsv, fmtData, fmtDataCurta, DIAS_SEMANA, DIAS_CURTO, diaSemanaNum, addDias, fmtPct, pct, fmtNum, normalizar, debounce, modal } from './util.js';
...
export async function render(el, { cabecalho }) {
  ...
  el.innerHTML = cabecalho('Relatórios', 'Resumos por período, aluno, curso, dia e horário. Exporte em CSV (abre no Excel) ou imprima.',
    `<button class="btn" id="rel-imp">${ico('imprimir')} Imprimir</button><button class="btn" id="rel-csv">${ico('baixar')} Exportar CSV desta aba</button>`) + `
    ...
    <div class="abas" id="rel-abas">
      ${[['resumo', 'Resumo'], ['aluno', 'Por aluno'], ['curso', 'Por curso'], ['dia', 'Por dia'], ['horario', 'Por horário'], ['ocorrencias', 'Ocorrências']]
  ...
  function alunosFiltrados() {
    const c = $('#rel-curso').value, n = $('#rel-nivel').value;
    return alunos.filter((a) => (!c || a.curso === c) && (!n || a.nivel === n));
  }
  function aplicarFiltros() {
    const ids = new Set(alunosFiltrados().map((a) => a.id)), met = $('#rel-metodo').value, uteis = $('#rel-dias').value === 'uteis';
    R = bruto.filter((r) => ids.has(r.a) && (!met || r.m === met) && (!uteis || diaSemanaNum(r.dt) <= 5));
    $('#rel-titulo-imp').textContent = `PASES Refeições · ${periodo.rotulo} (${fmtData(periodo.ini)} a ${fmtData(periodo.fim)})`;
    desenhar();
  }
  ...
  function tabelaAluno() {
    ...
    return base.map((a) => {
      ...
      return { id: a.id, nome: a.nome, matricula: a.matricula || '', curso: a.curso || '', ativo: a.ativo, total: rs.length,
  ...
  function tabelaCurso() {
    ...
    alunosFiltrados().forEach((a) => { const k = a.curso || '(sem curso)'; if (!porCurso.has(k)) porCurso.set(k, { curso: k, ativos: 0, atendidos: new Set(), total: 0, fora: 0, semfoto: 0 }); if (a.ativo) porCurso.get(k).ativos++; });
    R.forEach((r) => { const a = A.get(r.a); const k = a?.curso || '(sem curso)'; const c = porCurso.get(k); if (!c) return; c.total++; c.atendidos.add(r.a); if (!r.dh) c.fora++; if (r.fs === 'sem_foto') c.semfoto++; });
  ...
  function tabelaDia() {
    ...
    while (d <= periodo.fim) {
      ...
      if (rs.length || diaSemanaNum(d) <= 5) out.push({ data: d, dia: DIAS_SEMANA[diaSemanaNum(d)], total: rs.length, fora: rs.filter((r) => !r.dh).length,
  ...
  function desenhar() {
    ...
    else if (aba === 'curso') c.innerHTML = tabelaHtml(['Curso', 'Ativos', 'Atendidos', 'Refeições', 'Média por aluno atendido', 'Fora do horário', 'Sem foto'],
      tabelaCurso().map((x) => [esc(x.curso), x.ativos, `${x.atendidos} <small class="mudo">(${fmtPct(x.atendidos, x.ativos)})</small>`, x.total, x.media.toFixed(1).replace('.', ','), x.fora, x.semfoto]), [1, 2, 3, 4, 5, 6]);
    else if (aba === 'dia') {
      const t = tabelaDia();
      c.innerHTML = `<div class="cartao" style="margin-bottom:16px"><h2>Refeições por dia</h2>${barras(t.map((x) => ({ r: fmtDataCurta(x.data), v: x.total, dica: `${DIAS_CURTO[diaSemanaNum(x.data)]} ${fmtData(x.data)}: ${x.total}` })), { titulo: 'Refeições por dia' })}</div>` +
        tabelaHtml(['Data', 'Dia', 'Refeições', 'Facial', 'CPF', 'Manual', 'Fora do horário', 'Sem foto', 'Primeiro', 'Último'],
          t.map((x) => [fmtData(x.data), x.dia, `<b>${x.total}</b>`, x.facial, x.cpf, x.manual, x.fora, x.semfoto ? `<span class="selo vermelho">${x.semfoto}</span>` : 0, x.primeiro, x.ultimo]), [2, 3, 4, 5, 6, 7]);
    } else if (aba === 'horario') {
  ...
  function resumo() {
    ...
    return `<div class="kpis">
        <div class="kpi"><span>Refeições</span><b>${fmtNum(R.length)}</b><small>${dias.length} dia(s) com atendimento</small></div>
        ...
        <div class="kpi ${fora ? 'kpi-aviso' : ''}"><span>Fora do horário</span><b>${fora}</b><small>${fmtPct(fora, R.length)} · janela ${esc(cfg.horario_inicio)}–${esc(cfg.horario_fim)}</small></div>
      ...
      <div class="duas-col">
        <div class="cartao"><h2>Refeições por dia</h2>${barras(pd.map((x) => ({ r: fmtDataCurta(x.data), v: x.total, dica: `${DIAS_CURTO[diaSemanaNum(x.data)]} ${fmtData(x.data)}: ${x.total}` })), { titulo: 'Refeições por dia' })}</div>
  ...
  function desenharAlunos() {
    ...
    $('#rel-tab-aluno').innerHTML = `<p class="mudo pequeno" style="margin-top:0">${t.length} aluno(s). Frequência = refeições ÷ ${dias} dia(s) com atendimento no período.</p>` +
      tabelaHtml([['nome', 'Aluno'], ['curso', 'Curso'], ['total', 'Refeições'], ['freq', 'Frequência'], ['fora', 'Fora do horário'], ['semfoto', 'Sem foto'], ['facial', 'Facial'], ['ultima', 'Última']],
        t.map((x) => [`${esc(x.nome)}${x.ativo ? '' : ' <span class="selo">inativo</span>'}<br><small class="mudo">${esc(x.matricula)}</small>`, `<span class="pequeno">${esc(x.curso)}</span>`,
          `<b>${x.total}</b>`, `<div style="display:flex;align-items:center;gap:8px;justify-content:flex-end"><span style="display:inline-block;width:60px;height:6px;border-radius:3px;background:#eef1f0;overflow:hidden"><span style="display:block;height:100%;width:${Math.min(100, x.freq)}%;background:${x.freq < 50 ? '#e0a33a' : 'var(--verde)'}"></span></span>${String(x.freq).replace('.', ',')}%</div>`,
          x.fora, x.semfoto ? `<span class="selo vermelho">${x.semfoto}</span>` : 0, x.facial, x.ultima ? fmtData(x.ultima) : '<span class="mudo">nunca</span>']), [2, 3, 4, 5, 6], null, t.map((x) => x.id));
  ...
  function historico(id) {
    ...
    modal({ titulo: `${a.nome} · ${periodo.rotulo}`, largo: true, corpo: `<p class="mudo" style="margin:0">${esc(a.matricula || '')} · ${esc(a.curso || '')} · ${rs.length} refeição(ões) no período</p>` +
      tabelaHtml(['Data', 'Dia', 'Hora', 'Método', 'Foto', 'Horário'], rs.slice().reverse().map((r) => [fmtData(r.dt), DIAS_SEMANA[diaSemanaNum(r.dt)], r.h.slice(0, 5), METODO[r.m],
        r.fs === 'sem_foto' ? `<span class="selo vermelho">sem foto</span> <small>${esc(r.j || '')}</small>` : 'ok', r.dh ? 'dentro' : '<span class="selo ambar">fora</span>']), []) });
  }
  ...
  function exportar() {
    ...
    if (aba === 'aluno' || aba === 'resumo') baixarCsv(nome, ['aluno', 'matricula', 'curso', 'ativo', 'refeicoes', 'frequencia_%', 'fora_do_horario', 'sem_foto', 'facial', 'ultima'],
      tabelaAluno().sort((a, b) => a.nome.localeCompare(b.nome)).map((x) => [x.nome, x.matricula, x.curso, x.ativo ? 'sim' : 'não', x.total, String(x.freq).replace('.', ','), x.fora, x.semfoto, x.facial, fmtData(x.ultima)]));
    else if (aba === 'curso') baixarCsv(nome, ['curso', 'ativos', 'atendidos', 'refeicoes', 'media_por_atendido', 'fora_do_horario', 'sem_foto'],
      tabelaCurso().map((x) => [x.curso, x.ativos, x.atendidos, x.total, x.media.toFixed(2).replace('.', ','), x.fora, x.semfoto]));
    else if (aba === 'dia') baixarCsv(nome, ['data', 'dia', 'refeicoes', 'facial', 'cpf', 'manual', 'fora_do_horario', 'sem_foto', 'primeiro', 'ultimo'],
      tabelaDia().map((x) => [fmtData(x.data), x.dia, x.total, x.facial, x.cpf, x.manual, x.fora, x.semfoto, x.primeiro, x.ultimo]));
  ...
  ['#rel-curso', '#rel-nivel', '#rel-metodo', '#rel-dias'].forEach((s) => ($(s).onchange = aplicarFiltros));
